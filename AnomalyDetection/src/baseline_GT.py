...
@click.argument('net_name', type=click.Choice(['wrn']))
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model,
    ...
    print(dataset.train_set.data.shape)
    #dataset.train_set.data = np.pad(dataset.train_set.data, ((0, 0), (2, 2), (2, 2)), 'constant')
    ...
    print(dataset.test_set.data.shape)
    # Initialize DeepSAD model and set neural network phi
    gt = GT()
    gt.set_network(net_name, rep_dim=1)

    # If specified, load Deep DR model (center c, network weights, and possibly autoencoder weights)
    if load_model:
        gt.load_model(model_path=load_model, load_ae=True, map_location=device)
    ...
    # Test model
    gt.test(dataset, device=device, n_jobs_dataloader=n_jobs_dataloader)

    # Save results, model, and configuration
    gt.save_results(export_json=xp_path + '/results.json')
    gt.save_model(export_model=xp_path + '/model.tar')
