...
class D3REnnuLSIFTrainer(BaseTrainer):

    def __init__(self, pi, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 150,
    ...
    def train(self, dataset: BaseADDataset, net: BaseNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in train_loader:
                ...
                loss_negative = torch.sum(gu*unlabeled)/n_unlabeled - self.pi*torch.sum(gu*positive)/n_positive
    ...
    def test(self, dataset: BaseADDataset, net: BaseNet):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                outputs = net(inputs)
                density_ratio = torch.sigmoid(outputs)
                #losses = torch.where(semi_targets == 0, dist, self.eta * ((dist + self.eps) ** semi_targets.float()))
                #loss = torch.mean(losses)
                scores = -density_ratio
