...
def main():
    ...
    for i in range(ite):
        ...
        x_test_de = x_train
        x_test_nu = x_test[t_test==1]

        x_data = np.concatenate([x_test_nu, x_test_de], axis=0)
        
        t_test_nu = np.ones(len(x_test_nu))
        t_test_de = np.zeros(len(x_test_de))
        t_data = np.concatenate([t_test_nu, t_test_de], axis=0)
        ...
        train_pu, test_pu, auc_pu, mean_pu = train(x_train, t_train, x_test, t_test, epoch, model_pu, optimizer_pu, device, batchsize=batchsize, method='PU')
        train_ulisf, test_ulsif, auc_ulsif, mean_ulsif = train(x_train, t_train, x_test, t_test, epoch, model_ulsif, optimizer_ulsif, device, batchsize=batchsize, method='uLSIF')
        train_kliep, test_kliep, auc_kliep, mean_kliep = train(x_train, t_train, x_test, t_test, epoch, model_kliep, optimizer_kliep, device, batchsize=batchsize, method='KLIEP')

        train_nnpu, test_nnpu, auc_nnpu, mean_nnpu = train(x_train, t_train, x_test, t_test, epoch, model_nnpu, optimizer_nnpu, device, batchsize=batchsize, method='nnPU')
        train_nnulisf, test_nnulsif, auc_nnulsif, mean_nnulsif = train(x_train, t_train, x_test, t_test, epoch, model_nnulsif, optimizer_nnulsif, device, batchsize=batchsize, method='nnuLSIF')

        train_boundedulisf, test_boundedulsif, auc_boundedulsif, mean_boundedulsif = train(x_train, t_train, x_test, t_test, epoch, model_boundedulsif, optimizer_boundedulsif, device, batchsize=batchsize, method='boundeduLSIF')
